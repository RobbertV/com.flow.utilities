...
class App extends Homey.App {
    ...
    async action_START_DATE(name) {
        ...
        const newSettings = this.appSettings.CONVERSIONS.filter(
    ...
    async action_END_DATE(name) {
        const date = new Date();
        const existing_conversion = this.appSettings.CONVERSIONS.find(
            (x) => x.name === name
        );
        ...
        const duration = await this.calculateDiff(
            existing_conversion.date,
            date
        );

        const conversions = this.appSettings.CONVERSIONS.filter(
    ...
    calculateDiff(start, end) {
        ...
        const total = splitTime(diffInMilliseconds, this.homey.__);
