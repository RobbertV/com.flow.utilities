exports.init = async function (ctx) {
    ...
    action_END.registerArgumentAutocompleteListener('name', async (query, args) => {
        ...
        return totals.filter((totals) => totals && totals.name.toLowerCase().includes(query.toLowerCase()));
    });
    ...
    action_END_COMPARE.registerRunListener(async (args) => {
        ...
        await ctx.homey.app.action_END(args.name, null, args.value);
    });
    ...
    action_SET_CURRENCY.registerRunListener(async (args) => {
        ...
        const { currency } = args;
        await ctx.homey.app.action_SET_CURRENCY(currency, true);
    });
