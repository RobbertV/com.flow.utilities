...
exports.init = async function (ctx) {
    ...
    ctx.homey.app.trigger_ZONE_CHANGE = ctx.homey.flow
        .getTriggerCard(`trigger_ZONE_CHANGE`)
        .registerArgumentAutocompleteListener('zone', async (query) => zoneAutoComplete(ctx, query, true))
        .registerRunListener(async (args, state) => args.zone.id === state.zone && !!parseInt(args.onoff) === state.onoff);
